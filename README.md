...
- 关卡纸为横版 A4、四角定位标记；黑色横线（≥4cm）是地面，圆圈和三角旗帜分别标记起终点。
- 起点为纸上唯一圆圈；终点为唯一三角旗帜，支持黑色空心笔画。缺少起点或终点时生成失败，详见 [起终点规则](docs/level-markers.md)。
- 动作只有 `run`、`jump`；服务输出**透明 PNG 精灵表**，Unity 按帧播放，运行时不播 GIF/MP4。
- 唯一成功条件：角色接触旗帜。生成阶段不判断关卡可玩性或起终点可达性。
...
| `pg-worker` | 同 api 镜像 | — | 消费队列、子进程渲染 | `RENDER_USE_MESA=true`；`RENDER_MAX_DIM=800`（渲染前最长边，降低 ARAP 网格规模）；`ANALYZE_TIMEOUT_SECONDS=30`（仅分析总阶段，超时按基础设施错误重试）；`stop_grace_period: 300s`（≥120s×2 次尝试，防 docker 默认 10s SIGKILL 打断渲染） |
